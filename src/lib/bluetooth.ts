...
export async function connectFTMS(device: BluetoothDevice) {
	try {
		...
		if (cpService) {
			const cpChar = await cpService.getCharacteristic(
			...
			cpChar.addEventListener('characteristicvaluechanged', (ev: Event) => {
				...
				if (dv.byteLength >= 4) {
					...
					ftmsStore.updateData({ power: instPower });
				}
			});
		}
		...
		if (fitnessService) {
			const ibChar = await fitnessService
				.getCharacteristic(INDOOR_BIKE_DATA_CHARACTERISTIC_UUID)
				.catch(() => null);
			if (ibChar) {
				_ftmsChar = ibChar;
...
export async function connectHR(device: BluetoothDevice) {
	try {
		...
		device.addEventListener('gattserverdisconnected', () => {
			hrDevice.set(null);
			_hrChar = null;
			ftmsStore.updateData({ heartRate: null });
		});
		...
		hrChar.addEventListener('characteristicvaluechanged', (ev: Event) => {
			...
			ftmsStore.updateData({ heartRate: hr });
		});
	} catch (error) {
